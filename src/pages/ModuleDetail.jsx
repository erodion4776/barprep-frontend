...
function buildGreeting(moduleData, progress) {
  ...
  if (stats.totalAttempts > 0) {
    msg += `📊 Your accuracy is **${stats.overallAccuracy}%**. `
    if (isWeak) {
      msg += `**${moduleData.topic}** is a focus area — let's go deep!\n\n`
    } else {
      msg += `\n\n`
    }
  }
  msg += `I can explain concepts, quiz you, or break down key rules. What would you like to know?`
  return msg
}
...
function LockedAIChat({ topic }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center
                    p-5 text-center space-y-4 bg-gradient-to-b
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1.5 leading-relaxed
                      max-w-[200px] mx-auto">
          The video is free to watch. Upgrade to unlock the AI coach
      ...
      <div className="w-full bg-white border border-slate-200 rounded-xl
                      p-3 space-y-2 text-left">
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">
          With Pro you can:
        </p>
        {[
          { icon: '💬', text: 'Ask questions about this video'       },
          { icon: '📝', text: 'Get quizzed on key concepts'          },
          { icon: '⚖️', text: 'Get legal rule breakdowns'            },
          { icon: '🎯', text: `Get personalized tips for ${topic}`   },
        ].map(({ icon, text }) => (
          <p key={text} className="text-[11px] text-slate-600 flex items-start gap-1.5">
...
export default function ModuleDetail() {
  ...
  // Video is ALWAYS free — only AI chat is gated
  const canUseAIChat = canUse('tutorialDetail')
  ...
  // ── Load module ────────────────────────────────────────────────────────────
  // NOTE: No progress.loading guard — videos must load for ALL users
  ...
  const loadModule = useCallback(async () => {
    ...
    try {
      ...
      // Mark as watched — only if user is logged in
      try {
        ...
        // Silent — not critical for video playback
      }

      // Build AI greeting only if Pro user and no saved chat
      if (canUseAIChat) {
        const saved = sessionStorage.getItem(`module_chat_${id}`)
        if (!saved) {
  ...
  }, [id, markModuleWatched, canUseAIChat])

  // ── Only re-run when id changes — not on progress/plan changes ────────────
  useEffect(() => {
    loadModule()
  }, [id])
  ...
  const askQuestion = useCallback(async (e) => {
    ...
    if (!text || chatLoading || !module || !canUseAIChat) return
    ...
    try {
      ...
      const contextMsg = [
        ...
        module.ai_summary
          ? `[Summary: ${module.ai_summary.substring(0, 150)}]`
          : '',
      ...
      setChatHistory([...withUser, { role: 'assistant', content: errContent }])
    } finally {
      setChatLoading(false)
      inputRef.current?.focus()
    }
  }, [question, chatLoading, module, chatHistory, id, canUseAIChat, progress.stats])
  ...
  const smartQuickQuestions = useMemo(() => {
    if (!module || !canUseAIChat) return []
    return [
      isWeakTopic ? `Explain ${module.topic} simply` : 'Summarize this lecture',
  ...
  }, [module, isWeakTopic, canUseAIChat])
    ...
    <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-0">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              {topicPerf?.attempts > 0
                ? `You're at ${topicPerf.accuracy}% on ${module.topic}. Watch carefully!`
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* Video player — always free */}
          <VideoPlayer url={module.video_url} title={module.title} />

          {/* Free video confirmation note */}
          <div className="flex items-center justify-between gap-3
            ...
            <div className="flex items-center gap-2 text-xs text-green-700 font-medium">
              <span>✅</span>
              <span>
                This video is <strong>free for all users</strong>.
                {isFree && ' Upgrade to Pro to unlock the AI Coach chat.'}
              </span>
            </div>
            {isFree && (
              <Link
                to="/pricing"
                className="shrink-0 px-3 py-1.5 bg-blue-600 text-white
                           text-xs font-bold rounded-lg hover:bg-blue-700
              ...
              {[
                {
                  ...
                  desc:  `Test ${module.topic} knowledge`,
                },
                {
                  to:    '/chat',
                  icon:  '🤖',
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-slate-200 rounded-2xl
              ...
              <p className="text-xs mt-0.5">
                {canUseAIChat ? (
                  ...
                  <span className="text-blue-600 font-medium">
                    🔒 Pro feature — video is free to watch
                  </span>
                )}
              </p>
            </div>
            ...
            {!canUseAIChat ? (
              <LockedAIChat topic={module.topic} />
            ) : (
              <>
                {/* Messages */}
