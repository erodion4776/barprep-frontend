...
export const apiClient = {
  ...
  // ── Affirmation ───────────────────────────────────────────────────────────
  // 1. Read from Supabase DB (pre-generated by GitHub Actions cron daily)
  // 2. Fallback to Edge Function if no entry for today
  // 3. Hardcoded fallback if everything fails
  getAffirmation: async () => {
    const today = new Date().toISOString().split('T')[0]

    // Method 1: Read from database (instant — no cold start)
    try {
  ...
  // ── Video Processing ──────────────────────────────────────────────────────
  // Now runs on Supabase Edge Function — no more Render cold starts!
